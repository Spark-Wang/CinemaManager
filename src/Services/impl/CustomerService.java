...
public class CustomerService extends Service implements TicketHandler {
    ...
    public void order() {
        ...
        int i;
        for(i = 0;i < movieNameList.size();i ++){
            if(movieName.equals(movieNameList.get(i)))
                break;
        }
        if(i == movieNameList.size()){
...
//        printSeat(seat, hallLine, hallRow);
        ...
        if(count > (hallList.get(0).getH_capacity() - ticketList.size())){
            ...
            while(true){
                ...
                int c = inputCheck();
                if(c == 1){
                    selfChoose(scenceList.get(choice - 1), count);
                    break;
                }else if(c == 2){
                    autoChoose(scenceList.get(choice - 1), count);
                    break;
                }else{
    ...
    public void showScence(Scence scence){
        HallDao hallDao = new HallDaoImpl();
    ...
    public void selfChoose(Scence scence,int count){
        HallDao hallDao = new HallDaoImpl();
        String []param = new String[]{scence.getH_id() + ""};
        List<Hall> hallList = hallDao.getHall("select * from hall where h_id = ?",param);
        int hallLine = hallList.get(0).getH_line();
        ...
        for(int num = 0;num <count;num ++){
            ...
            int j = inputCheck();
            while(i > hallLine || i < 0 || j > hallRow || j < 0){
            ...
            sql = "insert into ticket (C_id, M_id, H_id, H_line, H_row, T_time) values (?,?,?,?,?,?)";
            Object[] params = new Object[]{super.getC_id(),scence_mid,scence_hid,i,j,s_timestamp};
    ...
    public void autoChoose(Scence scence, int count){
        HallDao hallDao = new HallDaoImpl();
        List<Hall> hallList = new ArrayList<>();
        String []param = new String[]{scence.getH_id() + ""};
        hallList = hallDao.getHall("select * from hall where h_id = ?",param);
        int hallLine = hallList.get(0).getH_line();
        int hallRow = hallList.get(0).getH_row();
        char[][] seat = new char[hallLine][hallRow];
        for(int l = 0;l < hallLine;l ++){
    ...
    public void changeTicket() {
        ...
        if(scenceList.size() == 1){
            ...
            selfChoose(scenceList.get(choice - 1),1);
        }
    }
